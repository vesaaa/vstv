...
### 4. 如何使用 rtp2httpd 作为直播源？
1. 在 `rtp2httpd` 侧启用播放列表输出（例如 `http://<网关IP>:5140/playlist.m3u`）。
2. 在 VsTV 的直播源中直接填写该 M3U URL。
3. 若列表中频道地址是相对路径（如 `/rtp/...`、`./xx.ts`），VsTV 会自动按订阅地址做绝对化处理后播放。

### 5. 长按确认键：快速切换直播源（约 v2.1.11+）
