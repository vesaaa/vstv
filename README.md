...
- **直播源兼容**：面向 **M3U 等主流 IPTV 订阅**，兼容社区常用的稳定列表（如 **Mursor**、**裤佬** 等相对稳定的公开源思路）；也可在设置或网页中换成你自己的地址。订阅中带 **`tvg-logo`** 时，选台列表与信息条会 **自动加载台标**（Coil 缓存）。
- **节目单开箱即用**：内置 **APTV 的 EPG** 与 **老张的 EPG** 两套默认模板；支持 **XML** 与 **gzip（.gz）** 拉取，无需手工解压。开启 EPG 后由 **WorkManager** 在后台按间隔自动刷新缓存（需联网），与「未到设定钟点前不拉取」的规则一致。
