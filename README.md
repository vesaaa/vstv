...
本应用 **不依赖** Google Play 服务或谷歌账号；网络播放使用 **OkHttp + Media3**，设置页走本机 **HTTP 服务（1616 端口）**。已做或请注意：
...
手机/电脑浏览器打开 **`http://<电视IP>:1616`** 可配置直播源、节目单等；页面会标明当前默认直播源、生效的节目单地址（含是否使用内置默认），并可在历史记录中点 **设为默认**。
